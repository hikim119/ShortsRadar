...
.dock{position:fixed;top:0;right:0;height:100vh;width:400px;background:#101018;
  ...
  touch-action:none}   /* 독 어디서든(영상 화면 제외) 스와이프가 스크롤로 새지 않게 */
...
/* 자동재생이 음소거로 시작된 경우 표시되는 소리 켜기 버튼 */
#unmute{position:absolute;left:10px;bottom:12px;z-index:5;display:none;
...
<div class="filters">
  ...
  <div class="frow"><span class="flabel">핫존</span><span id="hots"></span></div>
...
<aside class="dock" id="dock">
  <div class="dclip">
    ...
    <button id="unmute" onclick="return doUnmute()">🔇 소리 켜기</button>
  </div>
...
let win=3, bkt=0, srt="v", chn=0, pfl=0, hzf=2;   // 기본: 7일·핫존 20만+·조회수순 (AI 영상은 항상 제외)
...
// 핫존은 게시 1~3일 뒤에야 판정되는 영상이 대부분 → 켜면 기간을 7일로 자동 확장
function setHzf(i){hzf=i;if(i&&win<3)win=3;render();}
...
function cardHTML(d,i){
  ...
  return `<a class="card" id="c-${d.i}" href="${url}" target="_blank"
    ...
    <div class="body"><div class="title">${d.t.replace(/</g,"&lt;")}</div>
      ...
      <div class="stats"><span class="views">${fmt(d.v)}</span>${g}${hz}${nw}${st}${pc}
...
// 모바일 자동재생 차단 대응: 재생이 안 붙으면 ① 소리 그대로 재시도 → ② 그래도 안 되면
// 음소거로 재생 + 소리 켜기 버튼. 소리 자동재생이 되는 기기(안드로이드 등)는 ①에서 끝나고,
// 진짜 차단하는 기기(iOS)만 ②로 감. 버튼 한 번 탭 후엔 플레이어 재사용으로 소리 유지
let playT=0;
function watchPlay(){
  clearTimeout(playT);
  playT=setTimeout(()=>{
    if(curMode!=="y"||!ytp||!ytp.getPlayerState)return;
    let s=ytp.getPlayerState();     // 1=재생중 3=버퍼링
    if(s===1||s===3)return;
    try{ytp.playVideo();}catch(_){} // ① 소리 유지한 채 재시도 (느린 로딩이었으면 이걸로 재생)
    playT=setTimeout(()=>{
      if(curMode!=="y"||!ytp)return;
      s=ytp.getPlayerState();
      if(s!==1&&s!==3){try{ytp.mute();ytp.playVideo();showUnmute();}catch(_){}}  // ② 음소거 폴백
    },1200);
  },1600);
}
...
function doUnmute(){
  if(curMode==="y"){if(ytp){ytp.unMute();ytp.playVideo();}}
  else{const f=document.querySelector("#tslot iframe");
    if(f)f.contentWindow.postMessage({"x-tiktok-player":true,type:"unMute"},"https://www.tiktok.com");}
  hideUnmute();
  return false;
}
...
function onYTReady(){
  ...
  ytp=new YT.Player("pframe",{width:"100%",height:"100%",videoId:pendingId,
    ...
    events:{onReady:e=>{e.target.playVideo();watchPlay();},   // 자동재생 확실히
...
function openDock(id){
  ...
  if(d&&d.f==="t"){                      // 🎵 틱톡 → 틱톡 슬롯 (YT 플레이어는 살려둠)
    ...
    // 터치 기기는 제스처 없는 unMute가 무시될 수 있음 → 소리 켜기 버튼 노출
    if(matchMedia("(hover:none)").matches)showUnmute();
...
document.getElementById("dock").addEventListener("touchend",e=>{
  ...
  tY=null;});
document.getElementById("dock").addEventListener("touchmove",
  e=>e.preventDefault(),{passive:false});   // iOS: 독 위 스와이프가 스크롤로 새는 것 방지
