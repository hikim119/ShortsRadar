...
window.addEventListener("message",ev=>{
  ...
  if(m&&m["x-tiktok-player"]&&m.type==="onPlayerReady")
...
function openDock(id){
  ...
  pendingId=id;
  if(d&&d.f==="t"){                      // 🎵 틱톡 → 틱톡 임베드
    if(ytp){try{ytp.destroy();}catch(_){} ytp=null;}
    tiktokEmbed(id.slice(3));            // "tt_" 접두사 제거
  }else if(apiDead)plainEmbed(id);
  else if(ytp)ytp.loadVideoById(id);
  else if(window.YT&&YT.Player)onYTReady();   // 틱톡 재생 후 YT 복귀 등
  else loadYT();
}
function nav(dir){
...
function closeM(){
  ...
  else resetFrame("");   // 폴백/틱톡 모드: iframe 제거로 정지
  document.querySelectorAll(".card.sel").forEach(c=>c.classList.remove("sel"));
}
