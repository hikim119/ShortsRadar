...
// 모바일 자동재생 차단 대응: 1.5초 내 재생 시작이 없으면 음소거로라도 재생 + 소리 켜기 버튼.
// 버튼 탭(사용자 제스처)으로 unMute하면 플레이어를 재사용하므로 이후 영상은 계속 소리 유지
let playT=0;
function watchPlay(){
  clearTimeout(playT);
  playT=setTimeout(()=>{
    if(curMode!=="y"||!ytp||!ytp.getPlayerState)return;
    const s=ytp.getPlayerState();   // 1=재생중 3=버퍼링
    if(s!==1&&s!==3){try{ytp.mute();ytp.playVideo();showUnmute();}catch(_){}}
  },1500);
}
